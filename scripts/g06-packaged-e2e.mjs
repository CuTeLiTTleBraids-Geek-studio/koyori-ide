...
async function verifiedCLI(workDirectory, pin) {
  ...
  return {
    ...
    installLog: path.basename(installLog),
  };
}
async function waitForHandshake(file, child) {
  ...
  while (Date.now() < deadline) {
    ...
    if (child.exitCode !== null || child.signalCode !== null)
      throw new Error(`artifact exited before handshake (${child.exitCode})`);
    try {
...
async function captureWindow(pid, role, outputPath) {
  if (process.platform !== "win32")
    throw new Error("G06 packaged capture requires Windows");
...
`;
  const result = spawnSync(
    ...
    ["-NoProfile", "-NonInteractive", "-Command", script],
    {
      ...
      windowsHide: true,
      env: hostEnvironment({
        KOYORI_IDE_CAPTURE_PROCESS: String(pid),
  ...
  assert(metadata.sampledUniqueColours > 20);
  return {
...
async function sourceFingerprint() {
  const files = [
    ...
    "services/runtime_role.go",
    "services/runtime_role_e2e.go",
    "internal/e2e/server.go",
    "frontend/src/main.ts",
    "frontend/src/App.vue",
    "frontend/src/runtimeRole.ts",
    "frontend/src/e2e/runtimeRoleProbe.ts",
  ...
  for (const file of files) {
    ...
    details.push({ path: file, sha256: digest, bytes: content.length });
    hash.update(file);
    hash.update("\0");
    hash.update(digest);
    hash.update("\n");
...
if (dryRun) {
  ...
  console.log(
    "[g06-packaged-e2e] dry-run: no process launched; evidence remains U",
  );
  process.exit(0);
}
...
const workDirectory = await mkdtemp(
  path.join(os.tmpdir(), "koyori-ide-g06-e2e-"),
);
let launch;
let succeeded = false;
...
try {
  ...
  await runCommand(
    ...
    hostEnvironment({
      ...
      VITE_KOYORI_IDE_E2E_RUNTIME_ROLE: "1",
    }),
    buildLog,
  );
  const artifactInfo = await stat(artifact);
  assert(artifactInfo.size > 1024 * 1024);
  ...
  assert.equal(
    probe.main.ok,
    true,
    probe.main.error ?? "main role probe failed",
  );
  assert.equal(
    probe.aiFirst.ok,
    true,
    probe.aiFirst.error ?? "first AI role probe failed",
  );
  assert.equal(
    probe.aiReopen.ok,
    true,
    probe.aiReopen.error ?? "reopened AI role probe failed",
  );
  assert.equal(probe.main.role, "main");
  assert.equal(probe.aiFirst.role, "ai");
  assert.equal(probe.aiReopen.role, "ai");
  assert.equal(probe.main.forgedRole, "minimal");
  assert.equal(probe.aiFirst.forgedRole, "minimal");
  assert.equal(probe.aiReopen.forgedRole, "minimal");
  assert(probe.main.stages.includes("workflows"));
  for (const ai of [probe.aiFirst, probe.aiReopen])
    for (const forbidden of [
      ...
      assert(!ai.stages.includes(forbidden), `AI ran ${forbidden}`);
  assert.equal(probe.aiOpen, true);
  assert.equal(probe.aiVisible, true);
  assert.equal(probe.runtimeRole.resolvedMain, 1);
  assert.equal(probe.runtimeRole.resolvedAI, 2);
  assert(probe.runtimeRole.rejected >= 3);
  assert.equal(probe.runtimeRole.aiWindowsCreated, 2);
  assert.equal(probe.runtimeRole.aiWindowsClosed, 1);
  const screenshots = [
    await captureWindow(
      launch.child.pid,
      "main",
      path.join(evidenceDir, "g06-packaged-main.png"),
    ),
    await captureWindow(
      launch.child.pid,
      "ai",
      path.join(evidenceDir, "g06-packaged-ai.png"),
    ),
  ];
  ...
  const appProcesses = (Array.isArray(tree) ? tree : []).filter(
    (entry) => String(entry.Name).toLowerCase() === "koyori-ide.exe",
  );
  assert.equal(
    appProcesses.length,
    1,
    `expected exactly one koyori-ide.exe process, got ${appProcesses.length}: ${JSON.stringify(tree)}`,
  );
  await launch.flushLog();
  const evidence = {
    schemaVersion: 1,
    ...
    wailsCLI: {
      source: cli.source,
      sha256: cli.sha256,
      installLog: cli.installLog ?? null,
    },
    gitCommit: gitCommit(),
    gitMetadataAvailable: gitCommit() !== null,
    ...
    runtimeRoleProbe: probe,
    processTree: tree,
    screenshots,
    applicationLog: {
      file: "g06-packaged-launch.log",
      sha256: await sha256File(
        path.join(evidenceDir, "g06-packaged-launch.log"),
      ),
    },
    limitations:
      gitCommit() === null
        ? [
            "The workspace has an empty .git directory; no commit or CI history is claimed.",
          ]
        : [],
  };
  await writeFile(
  ...
  succeeded = true;
  console.log(`[g06-packaged-e2e] pass: ${path.relative(root, evidencePath)}`);
} catch (error) {
  const detail =
    error instanceof Error ? (error.stack ?? error.message) : String(error);
  await writeFile(
    failurePath,
    `${JSON.stringify({ schemaVersion: 1, goal: "P9-G06", status: "failed", evidenceLevel: "U", startedAt, failedAt: new Date().toISOString(), error: detail, retainedWorkDirectory: workDirectory }, null, 2)}\n`,
    "utf8",
  );
  console.error(`[g06-packaged-e2e] FAIL ${detail}`);
  process.exitCode = 1;
} finally {
  await stopProcess(launch);
  if (succeeded) await rm(workDirectory, { recursive: true, force: true });
  else console.error(`[g06-packaged-e2e] retained workspace=${workDirectory}`);
}
