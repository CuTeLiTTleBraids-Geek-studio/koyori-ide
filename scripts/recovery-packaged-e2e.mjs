...
async function sourceFingerprint() {
  ...
  const discovered = [
    ...await listFiles(path.join(root, "services"), (filePath) =>
      path.basename(filePath).startsWith("recovery") && filePath.endsWith(".go")),
    ...await listFiles(path.join(root, "internal", "e2e"), (filePath) => filePath.endsWith(".go")),
  ];
  ...
  for (const filePath of files) {
    ...
    manifest.push({ path: relativePath, sha256: digest, bytes: content.length });
...
async function stopProcessTree(launch) {
  if (!launch?.child?.pid || launch.child.exitCode !== null || launch.child.signalCode !== null) {
  ...
  if (launch.child.exitCode === null && launch.child.signalCode === null && process.platform !== "win32") {
    try {
...
async function forceTerminate(launch, reason) {
  assert(launch?.child?.pid, "force terminate requires a running packaged process");
  ...
  if (process.platform === "win32") {
    ...
    try {
      ...
      if (error?.code !== "ESRCH") throw error;
      result = { status: 0, signal: "SIGKILL", stdout: "already exited", stderr: "" };
    }
  }
  await waitForExit(launch.child, processStopTimeoutMilliseconds);
  assert(
...
async function runCommand(command, args, environment, logPath, cwd = root) {
  ...
  if (completed.code !== 0) {
    throw new Error(`${command} ${args.join(" ")} failed (code=${completed.code}, signal=${completed.signal})`);
  }
}
...
async function preparePinnedCLI(workDirectory, pin) {
  ...
  if (providedExecutable) {
    ...
    if (version.status !== 0 || reportedVersion !== pin) {
      throw new Error(`provided Wails CLI reported ${JSON.stringify(reportedVersion)}, want exactly ${pin}`);
    }
    return {
  ...
  const executable = path.join(cliDirectory, process.platform === "win32" ? "wails3.exe" : "wails3");
  const installLogs = [];
  let installError;
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    const buildLog = path.join(evidenceDirectory, `wails-cli-build-attempt-${attempt}.log`);
  ...
  const reportedVersion = `${version.stdout}${version.stderr}`.trim();
  if (version.status !== 0 || reportedVersion !== pin) {
    throw new Error(`temporary Wails CLI reported ${JSON.stringify(reportedVersion)}, want exactly ${pin}`);
  }
  return {
...
async function waitForJSON(filePath, child) {
  ...
  while (Date.now() < deadline) {
    if (child.spawnError) throw new Error(`process spawn failed: ${child.spawnError.message}`);
    if (child.exitCode !== null || child.signalCode !== null) {
      throw new Error(`process exited before handshake (code=${child.exitCode}, signal=${child.signalCode})`);
    }
    try {
...
async function launchArtifact({ artifact, cliDirectory, configDirectory, workDirectory, index }) {
  ...
  const launch = spawnCaptured(artifact, [], { env: environment, windowsHide: false });
  const flushLog = () => writeFile(logPath, launch.output(), "utf8");
  try {
    const handshake = await waitForJSON(handshakePath, launch.child);
    assert.equal(handshake.pid, launch.child.pid, "handshake PID does not match packaged process");
    await delay(1_000);
    assert.equal(launch.child.exitCode, null, "packaged process exited during startup settle");
    log("launch", `index=${index} pid=${handshake.pid} endpoint=${handshake.url}`);
    return {
      ...launch,
  ...
  try {
    const timeout = new Promise((_, reject) => {
      timeoutID = setTimeout(
        () => reject(new Error(`timed out waiting for recovery renderer mode ${mode}`)),
    ...
    assert.equal(result.ok, true, result.error ?? `renderer probe ${mode} failed`);
    return result;
  } finally {
...
async function journalSnapshot(configDirectory) {
  ...
  const files = await listFiles(recoveryRoot, (filePath) => filePath.endsWith(".json"));
  const records = [];
  const otherJSON = [];
...
async function captureWindowScreenshot(pid, outputPath) {
  if (process.platform !== "win32") {
    throw new Error("P9-G04 packaged screenshot capture is currently implemented for Windows only");
  }
...
`;
  ...
  if (result.status !== 0) {
    throw new Error(`window screenshot failed: ${result.stderr || result.stdout}`);
  }
  ...
  assert(metadata.sampledUniqueColours > 20, "window screenshot appears blank or one-colour");
  return {
...
async function verifyProductionRendererHookAbsent(environment) {
  ...
  const distFiles = await listFiles(path.join(root, "frontend", "dist"), (filePath) =>
    filePath.endsWith(".js") || filePath.endsWith(".html"));
  const forbidden = ["__koyoriIdeRunRecoveryProbe", recoveryResultEventMarker];
  for (const filePath of distFiles) {
    const content = await readFile(filePath, "utf8");
    for (const marker of forbidden) {
      assert(!content.includes(marker), `production frontend contains recovery E2E marker ${marker}`);
    }
  }
...
if (dryRun) {
  ...
  for (const required of [
    ...
    assert(source.includes(required), `recovery packaged driver is missing ${required}`);
  }
...
const failurePath = path.join(evidenceDirectory, "recovery-packaged-runtime.failure.json");
await Promise.all([rm(passPath, { force: true }), rm(failurePath, { force: true })]);
const workDirectory = await mkdtemp(path.join(os.tmpdir(), "koyori-ide-recovery-e2e-"));
const startedAt = new Date().toISOString();
let launch;
let succeeded = false;
...
try {
  ...
  const fixture = {
    ...
    crashContent: "export const diskVersion = 'original';\n// unsaved before first crash\n",
    pendingContent: "export const diskVersion = 'original';\n// edited while recovery was pending\n",
  };
  ...
  const productionHookAbsence = await verifyProductionRendererHookAbsent(baseEnvironment);
  const source = await sourceFingerprint();
  const commit = gitCommit();
  log("source", `fingerprint=${source.sha256} git=${commit ?? "unavailable"}`);

  const buildLogPath = path.join(evidenceDirectory, "recovery-packaged-build.log");
  await runCommand(cli.executable, ["build", "-tags", "e2e"], baseEnvironment, buildLogPath);
  ...
  const firstRenderer = await rendererProbe(launch.client, "prepare-crash", fixture);
  const firstJournal = await journalSnapshot(configDirectory);
  assert.equal(firstJournal.records.length, 1, "first crash preparation did not create exactly one journal record");
  ...
  const firstKill = await forceTerminate(launch, "crash after a dirty Monaco edit");
  launch = null;
  ...
  await launch.client.command("open-workspace", { workspace });
  const pendingRenderer = await rendererProbe(launch.client, "pending-check", fixture);
  ...
  assert.equal(secondJournal.records.length, 2, "pending edit was not isolated from the first crash snapshot");
  const preservedOriginal = secondJournal.records.find((record) =>
    record.windowId === pendingRenderer.originalWindowId);
  assert(preservedOriginal, "original crash snapshot disappeared during pending guards");
  assert.equal(preservedOriginal.sha256, firstJournal.records[0].sha256,
    "original crash snapshot changed during pending autosave/blur/close checks");
  assert(secondJournal.records.some((record) => record.content === fixture.pendingContent),
  ...
  if (Number.isFinite(visualPauseMilliseconds) && visualPauseMilliseconds > 0) {
    log("visual-pause", `pending window remains open for ${visualPauseMilliseconds}ms`);
    await delay(visualPauseMilliseconds);
  }
  const secondKill = await forceTerminate(launch, "second crash while recovery remained pending");
  launch = null;
  ...
  await launch.client.command("open-workspace", { workspace });
  const resolvedRenderer = await rendererProbe(launch.client, "resolve-save", fixture);
  ...
  assert.equal(await readFile(fixture.filePath, "utf8"), fixture.pendingContent);
  ...
  assert.equal(finalJournal.records.length, 0, "recovery records remained after explicit completion and manual save");
  ...
  const evidence = {
    ...
    launches: [
      { index: 1, pid: firstKill.pid, startedAt: firstRenderer.runId ? firstKill.startedAt : null },
      { index: 2, pid: secondKill.pid, startedAt: secondKill.startedAt },
      { index: 3, pid: launch.child.pid, startedAt: launch.handshake.startedAt },
    ],
    ...
    repeatedCrash: {
      ...
      originalSnapshotPreservedByteForByte: preservedOriginal.sha256 === firstJournal.records[0].sha256,
    },
  ...
  succeeded = true;
  log("pass", `artifact=${artifactSha256} evidence=${path.relative(root, passPath)}`);
} catch (error) {
  ...
  if (succeeded) {
    ...
    console.error(`[recovery-packaged-e2e] retained failure workspace: ${workDirectory}`);
  }
}
