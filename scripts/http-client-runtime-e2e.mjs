...
const publicURL = process.env.KOYORI_IDE_HTTP_CLIENT_PUBLIC_URL?.trim() || "https://example.com/";

if (!validModes.has(requestedMode)) {
  throw new Error(`invalid mode ${requestedMode}; expected all, dev, or packaged`);
}
...
async function sourceFingerprint() {
  ...
  const discovered = [
    ...await listFiles(path.join(root, "services"), (filePath) => path.basename(filePath).startsWith("http_client") && filePath.endsWith(".go")),
...
async function waitForJSON(filePath, child, expectedPID = null) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      if (expectedPID !== null) assert.equal(value.pid, expectedPID, "handshake PID does not match packaged process");
      return value;
    } catch (error) {
...
async function readBody(request, limit = 1024 * 1024) {
  ...
  for await (const chunk of request) {
    total += chunk.length;
    if (total > limit) throw new Error("HTTP probe request body exceeded limit");
    chunks.push(chunk);
  }
...
async function startHTTPFixtures(mode, rawLogPath) {
  ...
  const record = async (entry) => {
    const value = { sequence: records.length + 1, at: new Date().toISOString(), mode, ...entry };
    records.push(value);
  ...
  const secondary = http.createServer(async (request, response) => {
    ...
    await record({ server: "secondary", event: "request", method: request.method, url: request.url, headers: request.headers, body });
  ...
  const primary = http.createServer(async (request, response) => {
    const body = await readBody(request);
    await record({ server: "primary", event: "request", method: request.method, url: request.url, headers: request.headers, body });
    ...
    if (parsed.pathname === "/slow") {
      request.once("aborted", () => { void record({ server: "primary", event: "aborted", url: request.url }); });
      response.once("close", () => { void record({ server: "primary", event: "closed", url: request.url, finished: response.writableFinished }); });
      setTimeout(() => {
...
function validateHTTPLogs(records) {
  ...
  const pathCount = (prefix) => primary.filter((entry) => entry.url.startsWith(prefix)).length;
  assert.equal(pathCount("/guard/"), 0, "a fail-closed guard request reached the loopback server");
  assert.equal(pathCount("/response"), 1, "approved response endpoint call count");
  assert.equal(pathCount("/redirect-same"), 1, "same-origin redirect source call count");
  assert.equal(pathCount("/redirect-ok"), 1, "same-origin redirect target call count");
  assert.equal(pathCount("/redirect-cross"), 1, "cross-origin redirect source call count");
  assert.equal(pathCount("/slow?case=cancel"), 1, "cancellation endpoint call count");
  assert.equal(pathCount("/slow?case=timeout"), 1, "timeout endpoint call count");
  assert.equal(secondary.length, 0, "cross-origin private redirect reached the target server");
  ...
  try {
    const timeout = new Promise((_, reject) => {
      timeoutID = setTimeout(
        () => reject(new Error("timed out waiting for renderer HTTP-client command")),
...
async function preparePinnedCLI(workDirectory, pin) {
  ...
  if (version.status !== 0 || !`${version.stdout}${version.stderr}`.includes(pin)) {
...
function executionEnvironment(workDirectory, cliDirectory, handshakePath, token, configDirectory) {
...
async function runMode(mode, context) {
  ...
  const failurePath = path.join(evidenceDirectory, `${mode}-runtime.failure.json`);
  const applicationLogPath = path.join(evidenceDirectory, `${mode}-application.log`);
  ...
  try {
    ...
    let artifact;
    if (mode === "packaged") {
      log(mode, "building production artifact with the e2e-only renderer bridge");
      await runCommand(context.cli.executable, ["build", "-tags", "e2e"], environment, buildLogPath);
      artifact = path.join(root, "bin", process.platform === "win32" ? "koyori-ide.exe" : "koyori-ide");
      const info = await stat(artifact);
      assert(info.size > 1024 * 1024, "packaged artifact is unexpectedly small");
      ...
      launch = spawnCaptured(
        ...
        ["dev", "-config", "./build/config.yml", "-port", String(vitePort), "-nocolour"],
    ...
    assert.equal(rendererResult.ok, true, rendererResult.error ?? "renderer probe reported failure");
    ...
    const evidence = {
      ...
      limitations: context.gitCommit === null
    ...
    log(mode, `PASS evidence=${path.relative(root, passPath)} artifact=${evidence.artifactSha256}`);
    return evidence;
  } catch (error) {
    ...
    if (runtimePID !== null && runtimePID !== launch?.child.pid) await stopPIDTree(runtimePID);
...
const workDirectory = await mkdtemp(path.join(os.tmpdir(), "koyori-ide-http-client-e2e-"));
let succeeded = false;
try {
  ...
  log("source", `fingerprint=${source.sha256} git=${context.gitCommit ?? "unavailable"}`);
  if (requestedMode === "all" || requestedMode === "dev") await runMode("dev", context);
  if (requestedMode === "all" || requestedMode === "packaged") await runMode("packaged", context);
  succeeded = true;
} finally {
  if (succeeded) {
    const resolved = path.resolve(workDirectory);
    if (path.dirname(resolved) !== path.resolve(os.tmpdir()) || !path.basename(resolved).startsWith("koyori-ide-http-client-e2e-")) {
      throw new Error(`refusing to remove unexpected E2E directory ${resolved}`);
    }
    try {
      await removeTemporaryDirectory(resolved);
    } catch (error) {
      const modes = requestedMode === "all" ? ["dev", "packaged"] : [requestedMode];
      for (const mode of modes) {
        await rm(path.join(evidenceDirectory, `${mode}-runtime.json`), { force: true });
        await writeFile(path.join(evidenceDirectory, `${mode}-runtime.failure.json`), `${JSON.stringify({
          schemaVersion: 1,
          goal: "P9-G02",
          status: "failed",
          evidenceLevel: "U",
    ...
    console.error(`[http-client-runtime-e2e] retained failure workspace: ${workDirectory}`);
  }
}
