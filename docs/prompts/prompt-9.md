...
3. 测试 prerelease、build metadata 和非法版本。
...
- [x] 非法或无法映射的版本 fail-closed。`T`：sync 脚本对非 SemVer VERSION 直接 throw；`TestReleaseVersionConsistency_RejectsDrift`（空版本、非 SemVer、无版本字段）、`TestReleaseVersionConsistency_PrereleaseMSIXMapping`（prerelease 映射与错误四段拒绝）、`TestReleaseVersionConsistency_AcceptsBuildMetadata` 通过。
...
**现状与证据（2026-08-11 复核）：** 缺口已不存在。已实现 Dedicated Worker Extension Host（`frontend/src/lib/vscodeExtensionActivation.ts`）：ABI `1.0` 协商（`protocol-ready/protocol-error`、协商前 RPC 拒绝、未知/重协商版本拒绝）、随机 token 校验（伪造 token 忽略）、heartbeat watchdog（2s/8s）、消息配额（4 MiB/消息、1000 msg/s fail-closed）、Worker 内部 `error` 事件桥（发送认证 `runtime-error` 后关闭，弥补 WebView2 不转发 `Worker.onerror` 的行为）、crash/hang/rate/size 统一进入恢复路径、成功恢复清零连续失败计数。首次本轮 packaged 失败记录为 `disabled=true` 且 fault result 仍 `active=true`；后端 lifecycle stop handshake 已修复。低内存 full run 曾因 Git 启动 `0xc0000142` 失败，随后使用真实 `KOYORI_IDE_E2E_SKIP_BUILD=1` 复用既有 artifact 通过。最终 packaged manifest `build/e2e-evidence/packaged-e2e/manifest.json` 为 `status=passed`，24/24 fixtures 通过；artifact SHA-256 `7e8abff533098129f6cf858dd9278053c71786edbf5858a6003452589f07b181`，source fingerprint `690aa31cad880bf803037ab734207a9e1f7281d9e05140f65daaef15bd7b6180`，`recordedAt=2026-08-11T03:23:53.760Z`，manifest 明确 `gitMetadataAvailable=false`。G24 corpus 报告为 11/11 测试、10 包全 blocked（缺 `koyoriIde.permissions` 声明），无假成功。

**本轮最终门禁与环境记录：** Windows `node scripts/backend-gate.mjs` 最终 9/9、exit 0：gofmt 0.6s、vet 15.3s、build 14.5s、`go test ./... -count=1` 333.9s、contract 3.1s、bindings 12.6s，pin/docs 各检查为 0.x 秒。首次 Windows gate 红于 `TestLSPServiceRealTypeScriptWorkspaceLocalServer` 的 TempDir 被占用；根因是 `.cmd -> node` 后代未被 `Process.Kill` 回收，已在 `lspProcess.stop` 使用 `taskkill /PID /T /F` 并 `Wait` 修复，定向 TypeScript LSP 与 LSP 测试随后通过。WSL `.wslconfig` 已由 8GB 调整为 6GB 并启用 `autoMemoryReclaim=gradual`，配置生效后 `free` 显示 5.8GiB；true skip-build packaged 运行 24/24。最终已重建普通 production frontend，五个 E2E marker 扫描结果为 0。以上均为本地 Windows/WSL 证据，不声称 git、CI 或 release。
...
### AC
- [x] host 崩溃/卡死/超配额时主 IDE 可继续编辑保存并重启 host。`P`：最终 packaged 24/24 manifest 为 `passed`，包含 crash/hang/rate/size recovery、故障后 edit/save/open-file 与 host 重启验证；artifact SHA-256 `7e8abff533098129f6cf858dd9278053c71786edbf5858a6003452589f07b181`。
...
- [x] packaged 安装、激活、升级、禁用、卸载闭环通过。`P`：`build/e2e-evidence/packaged-e2e/manifest.json` status=`passed`，24/24 fixtures passed，artifact SHA-256 `7e8abff533098129f6cf858dd9278053c71786edbf5858a6003452589f07b181`，source fingerprint `690aa31cad880bf803037ab734207a9e1f7281d9e05140f65daaef15bd7b6180`。
...
| P9-G10 | 阻塞 | 3/4 | T/P/U | 2026-08-09 | Windows packaged 23/23 fixtures 通过（含真实 gopls、Settings CAS、Extension API、Debug、Test Explorer、Git worktree/rebase、AI receipt、终端重连 UI 与重启恢复）；测试 artifact SHA-256 `38b7d545c69535b72770f0cf544e95f6ee962db10b390cd580196eebde5cc410`，当前 Windows production artifact SHA-256 `36dc76ca58bcc0a66e9a4fd0a3fef3c3b7d184d9b94a01e261ecff28f5888774`；manifest/screenshot 已归档，production artifact 无 probe marker。AC2 macOS/Linux packaged 矩阵仍需外部 runner，当前不可达 |
...
| P9-G16 | 进行中 | 2/4 | T/I/P/U | 2026-08-09 | `terminal:exited` 扩展为 `sessionId/code/signal/err`，Unix PTY 真实 signal 解析；TerminalPanel 保存 cwd/shell 并提供 fail-closed 重连 UI，会话 ID 使用 UUID/随机字节加进程内单调序号防碰撞，store 24/24、TerminalPanel 29/29；Linux/WSL 默认与自定义 `sh`、真实 SIGTERM 通过；Windows pinned packaged `terminal-reconnect-package` 与完整 23/23 矩阵通过，artifact SHA-256 `38b7d545c69535b72770f0cf544e95f6ee962db10b390cd580196eebde5cc410`，source fingerprint `0bcd15bea4628ab0cf4e65914deeb9a4d36ab822aee74dd0e6846088852a07f5`；macOS runner/默认 shell/signal 仍为 U |
| P9-G17 | 完成 | 3/3 | T/I/P | 2026-08-09 | GitService 多根 roots + nested discovery、GitPanel 多根/嵌套 repo 选择（36/36）与 ProjectService 回滚接线完成；pinned packaged 23/23（含真实 rebase/worktree），artifact SHA-256 `38b7d545c69535b72770f0cf544e95f6ee962db10b390cd580196eebde5cc410`，manifest `build/e2e-evidence/packaged-e2e/manifest.json`，空 `.git` 仅用 fingerprint `0bcd15bea4628ab0cf4e65914deeb9a4d36ab822aee74dd0e6846088852a07f5`；下一项为 G18 |
| P9-G18 | 完成 | 4/4 | T/P | 2026-08-09 | commit receipt（TransactionID+FileHashes）、`committed-ui-sync-failed` fail-closed UI 状态、磁盘漂移/重复提交拒绝与重启恢复均已验证；当前 Windows packaged manifest 23/23，`g18ReceiptRecovery` 六项全为 `true`，artifact SHA-256 `38b7d545c69535b72770f0cf544e95f6ee962db10b390cd580196eebde5cc410`，manifest 与 exe 一致；Go receipt 定向测试、前端 12 测试和 driver 3/3 通过。 |
...
```text
严格按 docs/prompts/prompt-9.md 执行。先读第 0、1、2、3、8 节，复核进度板和当前代码；G24 已完成，按 Goal 顺序下一候选为 G25，但 G25 依赖 G23/G24，G23 AC2-4 仍为 `U`，不得宣称依赖全部满足或无条件开工。仅在依赖核验通过后选择依赖已满足的第一个“进行中”或“未开始”Goal，一次只推进这个 Goal；本轮不推进 G25 实现。开始前写明审计缺口“仍存在 / 已变化 / 已不存在”。所有 AC 必须以 S/T/I/P/R/U 分级，mock/contract 不得冒充真实集成、packaged 或 CI 证据；未勾选 AC 时不得宣称完成。安全修改先补绕过失败测试，用户数据修改先证明冲突、崩溃和回滚。不要手工猜 Wails binding ID，必须使用项目锁定版本生成。完成或暂停时按第 9 节交付，并立即回写 AC 与第 8 节进度板。不要 commit、push、tag 或发布，除非我明确要求。
```
